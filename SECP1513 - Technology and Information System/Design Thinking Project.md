# Design Thinking Project
Project Title : Smart Campus Data Solution - Improving Student Experience through Data
<br> Project Theme : Energy Usage and Sustainability

|Group Members|
|---|
Balqis Batrisya binti Jalaluddin
Shasya Shafieqah binti Shaharudin
Yong See En

## 🎯 Project Objective
To apply the Design Thinking process to identify, analyze, and prototype a data-driven solution that enhances the student experience or campus efficiency.
Students will learn to:
<br> •	Empathize with users through data collection and observation.
<br> •	Define and frame a meaningful data-related problem.
<br> •	Ideate and prototype a creative, data-based solution.
<br> •	Test and refine ideas based on feedback.

## Reflection
Throughout the process to finish this project, I got to learn many things including design thinking process, which consists of five phases. Empathy, define, ideate, prototype, and testing. These phases are really helpful in producing an effective solutions to the correspond problems, as it helped me to understand more about user needs and behaviours.

The emphatize and define phase helped me see the importance of observing real problems instead of making assumptions. It made me realize that small issues can lead to bigger impact when addressed properly. This change the way I approach problem-solving, as I now focus more on the user's perspective. During ideate and prototype phase, I improved my teamwork and communication skills by exchanging ideas, and refining our solution together. Although there were some challenges, this experience taught me how to adapt and work more effectively as a team.

Overall, this project improve my critical thinking, creativity, and awareness of sustainability issues. It encouraged me to think beyond theoretical knowledge and apply what I learned to practical problems. This experience is very valuable especially for future projects and academic work.

### View our work here:
<a href="https://youtu.be/LfIVa4f8p3s">Design Thinking video</a> <br>
<a href="https://www.canva.com/design/DAG-Fexf_rc/EvGIbeQojNdtYNsoc2BL4A/view?utm_content=DAG-Fexf_rc&utm_campaign=designshare&utm_medium=link2&utm_source=uniquelinks&utlId=he9e92e3d8b">Presentation Slide</a>
